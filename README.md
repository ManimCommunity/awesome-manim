...
[![Star History Chart](https://star-history.dera.page/svg?repos=ManimCommunity/awesome-manim&type=Date)](https://star-history.dera.page/#ManimCommunity/awesome-manim&Date)
...
2. Add your YouTube channel and other useful links (ie: Github, Patreon, Discord server, etc.), keeping the alphanumeric order of the list of creators. Make sure that your YouTube channel link follows any of these formats:
   ...
   If your link:
   - is missing the `https://` prefix,
   - it has a `http://` prefix instead,
   - it's missing the `www.`,
   - it includes `user` instead of `channel`, or
   - it's a link to a specific video rather than a YouTube channel,

   the website will not fetch any videos from there!
